...
const VERIFIER_REGISTRY_ADDRESS = import.meta.env.VITE_VERIFIER_REGISTRY_ADDRESS as `x0${string}`;
...
export interface VerifierStats {
  ...
  rewardsEarned: number; // in USDC (6 decimals)
}
 ...
 * Handles verifier registration, paper verification, and reward claiming
 */
export const useVerifierRegistry = () => {
  ...
  const registerVerifier = async () => {
    ...
    if (!VERIFIER_REGISTRY_ADDRESS) {
      return { success: false, error: "VerifierRegistry contract not configured" };
    }
   ...
   * Verify a paper
   */
  const verifyPaper = async (tokenId: number, approved: boolean, comment: string) => {
    if (!walletClient) {
      return { success: false, error: "Wallet not connected" };
    }
    ...
    try {
      ...
      toast.success("Paper verification submitted!", {
        id: toastId,
        description: `Tx Hash: ${tx.hash.slice(0, 10)}...${tx.hash.slice(-8)}`
      });
      ...
      let msg = error.message || "Unknown error";
      if (msg.includes("User rejected")) msg = "Transaction rejected by user";
      if (msg.includes("Not registered")) msg = "You are not registered as a verifier";
  ...
  /**
   * Claim verification reward (50 USDC per verified paper)
   */
  const claimReward = async (tokenId: number) => {
    ...
    try {
      ...
      toast.success("Reward claimed successfully! +50 USDC", {
        id: toastId,
        description: `Tx Hash: ${tx.hash.slice(0, 10)}...${tx.hash.slice(-8)}`
      });

      return { success: true, txHash: tx.hash, amount: 50 };
    } catch (error: any) {
      ...
      if (msg.includes("User rejected")) msg = "Transaction rejected by user";
      if (msg.includes("Not registered")) msg = "You are not registered as a verifier";
      if (msg.includes("No unclaimed reward")) msg = "No unclaimed reward found for this paper";
  ...
  const getVerifierStats = async (address: string): Promise<VerifierStats | null> => {
    if (!VERIFIER_REGISTRY_ADDRESS || !address) {
      return null;
    }
    ...
    try {
      ...
      return {
        tier: stats[0],
        totalVerifications: Number(stats[1]),
        correctVerifications: Number(stats[2]),
        rewardsEarned: Number(stats[3]), // in USDC (6 decimals)
      };
    } catch (error) {
  ...
  /**
   * Get verifications for a paper
   */
  const getPaperVerifications = async (tokenId: number): Promise<Verification[]> => {
    if (!VERIFIER_REGISTRY_ADDRESS) {
      return [];
    }
   ...
   */
  const getVerificationCount = async (tokenId: number): Promise<number> => {
    if (!VERIFIER_REGISTRY_ADDRESS) {
      return 0;
    }
  ...
  /**
   * Get unclaimed rewards for a verifier
   * Returns array of tokenIds with unclaimed rewards
   */
  const getUnclaimedRewards = async (verifierAddress: string): Promise<number[]> => {
    if (!VERIFIER_REGISTRY_ADDRESS || !verifierAddress) {
      return [];
    }
    ...
    try {
      ...
      // Get all verifications and check which ones have unclaimed rewards
      const unclaimed: number[] = [];

      // This is expensive - we'd need to query each paper
      // For now, return empty array - would need indexing service for efficient queries
      return unclaimed;
    } catch (error) {
