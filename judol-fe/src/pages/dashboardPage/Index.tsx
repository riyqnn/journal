...
// Map contract data to display format
const mapContractToDisplay = (paper: any) => ({
  id: paper.tokenId,
  title: paper.title,
  type: "Research IP",
  status: paper.status.toLowerCase() as 'verified' | 'processing' | 'data_pool',
  license: paper.license || "Commercial (PIL)",
  earnings: "0 USDC", // TODO: Track actual earnings from contract
...
export default function DashboardPage() {
    ...
    // Map contract assets to display format
    useEffect(() => {
        if (assets.length > 0) {
            setMappedAssets(assets.map(mapContractToDisplay));
        }
    }, [assets]);
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
                            <div className="bg-white border-2 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center justify-center min-w-[140px]">
                                ...
                                <span className="text-3xl font-black font-mono text-green-600">645 USDC</span>
                            </div>
                            <Link to="/mint">
                                            ...
                                            <div key={asset.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 hover:bg-yellow-50 transition-colors gap-6 group">
                                                ...
                                                <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                                                    <div className="text-right">
                                                        ...
                                                        <div className="font-black font-mono text-green-700 text-lg">{PRICING.getVerifiedPrice()}</div>
                                                    </div>
